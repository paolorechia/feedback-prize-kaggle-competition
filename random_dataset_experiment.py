...
from genetic_algorithm import GeneticAlgorithm, DatasetContext
import warnings
...
def main():
    ...
    for attribute in attributes:
        ...
        genetic = GeneticAlgorithm(
            ...
            first_ancestor=[float(x) for x in first_pred],
        )
        genetic.initialize_population()
        genetic.run(num_generations=epochs)
        best = genetic.best_individual

        train_df[attribute] = [fit_float_score_to_nearest_valid_point(x) for x in best]
